...
# Anything older than this is treated as stale and excluded from retrieval.
_FRESHNESS_DAYS = 365


class ChromaProductKnowledgeAdapter(ProductKnowledgePort):
    """Persistent ChromaDB vector store. Sprint 5: chunking now writes
    full metadata (delflag, upddate, prodcatversion) so the retrieval-side
    filter can hide deleted or stale products."""
    ...
    def find_matches(self, query: str, k: int) -> List[ProductMatch]:
        ...
        cutoff = int((datetime.now() - timedelta(days=_FRESHNESS_DAYS)).timestamp())
