...
def test_falls_back_to_env_var(isolated_env, monkeypatch):
...
def test_reset_flag_clears_keyring(isolated_env, monkeypatch):
    _patch_keyring(monkeypatch, stored="old-key")
    monkeypatch.setenv(api_key_provider.RESET_ENV_VAR, "1")
    monkeypatch.setattr(api_key_provider, "_prompt_for_key", lambda: "new-key")

    assert api_key_provider.get_wps_api_key() == "new-key"
